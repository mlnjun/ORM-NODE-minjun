...
router.post('/uploadprofile', upload.single('file'), async(req,res,next)=>{
    ...
    if(profile != null){
      // 데이터 존재
      // 파일 경로
      var filePath ="/upload/channel/"+profile.filename;
...
// 그룹 채팅 프로필 이미지 삭제 라우팅 메소드
// DELETE
// http://localhost:3000/api/channel/profileimg
router.delete('/profileimg', async(req,res,next)=>{
...
router.post('/addchatgroup', async(req,res,next)=>{
  ...
  try{
    // 데이터 받기
    var inviteMember = req.body.inviteMembers;
