...
// path라는 노드 프레임 워크의 파일/폴더 경로정보를 추출하는 패키지를 참조합니다.
var path = require('path');

// 웹서버에서 발급해주는 쿠키파일에 대한 정보를 추출하는 cookie-parser 패키지를 참조합니다.
var cookieParser = require('cookie-parser');

// morgan이라는 노드패키지를 통해 사용자 이벤트(요청과응답) 정보를 로깅(이력활동정보 저장)하는
// 패키지 참조
// 더 좋은 패키지 있음
...
// 매번 요청이 발생할 때마다 실행되는 어플리케이션 미들웨어 함수 기능정의 하기 
// 모든 app.use()메소드들은 특정 사용자의 요청과 으답이 발생할 때 마다 실행되는 기능
...
// 개발자 정의 라우팅 미드루에어
// localhost:3000/user/aaa
app.use('/user/:id',function(req, res, next){
  const uid = req.params.id;
  console.log('어플리케이션 미들웨어 호출2 :', req.method);  // 어떤 타입의 요청인지 나옴
  res.send("사용자 아이디 :"+uid);  //res가 실행되면 밑으로 가지않음
})

// 라우터 파일에 대한 기본 경로 설정하기
app.use('/', indexRouter);
app.use('/users', usersRouter);


// 사용자 요청에 대해 요청을 못찾거나 리소스를 못찾으면 404 에러를 웹브라우저에 전달해주는
// 미들웨어 함수
// catch 404 and forward to error handler
// 대부분 사용자 요정과 응답을 상기 기본/개발자정의 라우터파일에서 처리가 되어지고
// 처리되어지지 못한 사용자 요청은 여기 404미들웨어에서 처리됩니다.
// 우선순위가 높은 위쪽 코드에서 해당하는 라우터로 전달되지 못할 때 실행된다.
app.use(function(req, res, next) {
  next(createError(404));
});

// MVC패턴 노드 백엔드 환경에서의 서버에러 발생시 처리해주는 전역예외처리기 기능 제공
// error handler
